 ...
 * 3. Când o echipă are 4W la rând, verifică dacă e în top 50% la acel moment
 * 4. Dacă da, verifică al 5-lea meci
 */
...
function analyzeSeason(data) {
    ...
    for (const m of meciuri) {
        ...
        // și verifică dacă meciul CURENT (al 5-lea) e victorie
        for (const [team, result] of [[home, homeResult], [away, awayResult]]) {
            const hist = teamHistory[team];
            if (hist.length >= 4) {
                const last4 = hist.slice(-4);
                if (last4[0] === "W" && last4[1] === "W" && last4[2] === "W" && last4[3] === "W") {
                    // Echipa are 4W la rând! Verifică poziția ÎN CLASAMENT ACUM
                    const pos = getPosition(standings, team, allTeams);
                    if (pos <= halfPos) {
                        totalStreaks++;
                        if (result === 'W') wonFifth++;
                    }
                }
            }
        }
    ...
    return { totalStreaks, wonFifth, totalTeams, halfPos, totalMatches: meciuri.length };
}
...
function analyzeLeague(leaguePattern) {
    ...
    return { leagueName, leaguePattern, totalStreaks, wonFifth, files: files.length, seasonDetails };
}

// ==================== MAIN ====================
console.log('='.repeat(95));
console.log('ANALIZĂ: Echipă din TOP 50% clasament cu 4 victorii la rând → câștigă al 5-lea?');
...
for (const league of COMPLETE_LEAGUES) {
    ...
    const rate = r.totalStreaks > 0 ? ((r.wonFifth / r.totalStreaks) * 100).toFixed(1) : 'N/A';
    const lost = r.totalStreaks - r.wonFifth;
    console.log(`${r.leagueName.padEnd(45)} | 4W serii: ${String(r.totalStreaks).padStart(4)} | 5th W: ${String(r.wonFifth).padStart(4)} | 5th L/D: ${String(lost).padStart(4)} | Rata: ${String(rate).padStart(5)}%`);
}

console.log('');
console.log('='.repeat(95));
const grandRate = grandTotal > 0 ? ((grandWon / grandTotal) * 100).toFixed(1) : 'N/A';
console.log(`TOTAL GLOBAL (top 50%):  Serii 3W: ${grandTotal} | Al 5-lea W: ${grandWon} (${grandRate}%) | Al 5-lea L/D: ${grandTotal - grandWon}`);
console.log('='.repeat(95));
...
const sorted = results
    ...
    .sort((a, b) => (b.wonFifth / b.totalStreaks) - (a.wonFifth / a.totalStreaks));

for (let i = 0; i < sorted.length; i++) {
    const r = sorted[i];
    const rate = ((r.wonFifth / r.totalStreaks) * 100).toFixed(1);
    console.log(`${String(i + 1).padStart(2)}. ${r.leagueName.padEnd(45)} ${rate}%  (${r.wonFifth}/${r.totalStreaks})`);
}
...
for (const r of results) {
    ...
    for (const s of r.seasonDetails) {
        const rate = s.totalStreaks > 0 ? ((s.wonFifth / s.totalStreaks) * 100).toFixed(1) : 'N/A';
        console.log(`   ${s.sezon.padEnd(15)} | ${String(s.totalTeams).padStart(2)} echipe | Top ${String(s.halfPos).padStart(2)} | Meciuri: ${String(s.totalMatches).padStart(4)} | 3W: ${String(s.totalStreaks).padStart(3)} | 5th W: ${String(s.wonFifth).padStart(3)} | Rata: ${rate}%`);
    }
}
